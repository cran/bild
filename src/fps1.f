C CCCC
C  function to calculate the integral for psi1 for the second order 
C dependence model with random effects
C dimension of x and vectors assume the maximum values
CCCCC

      DOUBLE PRECISION FUNCTION fps1(v,i)
      DOUBLE PRECISION logL,prob,beta1,bt1,lpsi1,x1,
     *theta1,work1,omega1,v,d2,gbt,glps1,glps2,
     ...
     *der,db,db1,db2
      ...
      d2=glps1
      fps1 = dexp(logL-(v**2)/(2*dexp(omega1)))*d2
      return
      end
